...
public sealed class ConfigManager
{
    ...
    private void LoadAllAccounts() { try { foreach (var p in Directory.GetFiles(configDirectory, "*_LanParty.json")) { var a = JsonSerializer.Deserialize<AccountConfig>(File.ReadAllText(p), JsonOptions); if (a != null && !string.IsNullOrWhiteSpace(a.AccountId)) accounts[a.AccountId] = a; } } catch (Exception ex) { log.Error(ex, "[LAN Party] Failed to load account configs."); } }
    private void SaveAccount(string id) { if (!accounts.TryGetValue(id, out var a)) return; try { File.WriteAllText(Path.Combine(configDirectory, $"{id}_LanParty.json"), JsonSerializer.Serialize(a, JsonOptions)); } catch (Exception ex) { log.Error(ex, "[LAN Party] Failed to save account config."); } }
}
