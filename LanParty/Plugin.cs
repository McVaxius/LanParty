...
using LanParty.Windows;
using LanParty.Services;
...
public sealed class Plugin : IDalamudPlugin
{
    ...
    public Plugin()
    {
        ...
        SetupDtrBar();
        UpdateDtrBar();
        Log.Information("[LAN Party] Plugin loaded.");
    }
    ...
    {
        if (dtrEntry == null) return; dtrEntry.Shown = Configuration.DtrBarEnabled; if (!Configuration.DtrBarEnabled) return; var g = Configuration.PluginEnabled ? Configuration.DtrIconEnabled : Configuration.DtrIconDisabled; var s = Configuration.PluginEnabled ? "On" : "Off"; dtrEntry.Text = Configuration.DtrBarMode switch { 1 => new SeString(new TextPayload($"{g} LP")), 2 => new SeString(new TextPayload(g)), _ => new SeString(new TextPayload("LP: " + s)), }; dtrEntry.Tooltip = new SeString(new TextPayload($"{PluginInfo.DisplayName} {s}. Click to toggle."));
    }
